...
app.get('/api/auth/google/callback', (req, res, next) => {
    console.log('Google callback hit, config.appUrl:', config_1.config.appUrl);
    console.log('Google callback failure redirect:', `${config_1.config.appUrl}/login?error=google_auth_failed`);
    passport_1.default.authenticate('google', {
        failureRedirect: `${config_1.config.appUrl}/login?error=google_auth_failed`,
        failureMessage: true
    })(req, res, (err) => {
        if (err) {
            console.error('Passport authenticate error:', err);
            return res.redirect(`${config_1.config.appUrl}/login?error=auth_error`);
        }
        if (!req.user) {
            console.error('No user after Google auth');
            return res.redirect(`${config_1.config.appUrl}/login?error=no_user`);
        }
        ...
        const redirectUrl = `${config_1.config.appUrl}/auth/callback?token=${token}`;
